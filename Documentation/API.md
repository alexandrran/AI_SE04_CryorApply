...
## Generate CV Questions

Reads the CV and returns tailored follow-up questions whose answers would make
the CV stronger (for example: missing metrics, missing portfolio/LinkedIn/GitHub
links, thin project detail). Questions are generated by Gemini based on the
specific CV, following Harvard Career Services resume principles, with a
...
| `file` | `PDF` or `DOCX` file | No | CV uploaded by the user. Maximum size: 10 MB |
| `cv_text` | `string` | No | CV text pasted by the user. Required when no file is uploaded |
| `job_description` | `string` | No | Target role used to tailor the questions |
...
```json
{
  ...
  "questions": [
    {
      "id": "metrics",
      "question": "Your weather app project has no numbers. How many users or downloads did it get?",
      "reason": "Quantified results make experience far more convincing.",
      "placeholder": "e.g. used by 200+ students"
    },
    {
      "id": "github",
      "question": "Do you have a GitHub profile? Paste the link.",
      "reason": "For technical roles, GitHub shows your real code.",
...
Works with uploaded PDF/DOCX files as well as pasted text. Answers from the
questions step are woven in to strengthen the result. Rewriting follows Harvard
Career Services resume principles (action verbs, quantified results, no personal
pronouns, reverse chronological order).
...
| `answers` | `string` (JSON) | No | JSON array of `{ "question": "...", "answer": "..." }` from the questions step |
